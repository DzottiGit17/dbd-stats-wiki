...
## 2026-08-23 (14)
User asked "what else can we add or fix" then "fix all of them plz" — worked the full punch list: (1) filled the 2 missing killer-perks files (The Deathslinger, The Dark Lord) that made "44/44" inaccurate — genuinely 44/44 now. (2) Pulled survivor perk-breakdown data for all remaining 44 survivors via NightLight.gg — survivor coverage is now 53/53, matching killers. Found and fixed two slug mismatches along the way: `Bill Overbeck`'s data-source display name is "William \"Bill\" Overbeck" but the site's own data uses "Bill Overbeck" → file is `bill-overbeck.json` not `william-bill-overbeck.json`; `Leon Scott Kennedy` (site's name) vs "Leon S. Kennedy" (NightLight's name) → file is `leon-scott-kennedy.json`. Also caught `slugify("Élodie Rakoto")` stripping the "É" as non-alnum, same class of bug as the earlier "Onryō" one — produces `lodie-rakoto`, not `elodie-rakoto`. (3) Expanded `trivia.json` from 7 to 11 entries using real NightLight numbers already in `killers.json`/`survivors.json` (lowest kill rate, highest/lowest escape rate, and a caveat entry flagging Aurora Stardotter's 66.67% escape rate as a 24-game sample-size mirage, not a real signal). (4) Expanded `builds.json` from 1 to 3 entries: a killer "Slowdown Stack" meta build from the top-4 general killer perks, and a survivor "Boon Squad" fun build. (5) Verified `perk-info.json` (30 icon/tooltip entries) and `perks.json` (curated top-10/top-10 overview, sourced) were both already correct as designed, not gaps. All new/changed JSON validated as parseable. Remaining known gaps: perk pages have no per-perk detail view (list only), and the site's live "most prestiged" / hook-time stats still don't exist anywhere (confirmed earlier, not fabricated).

## 2026-08-21 (13)
Filled the perk-breakdown gap the user flagged ("why some killers have no data yet? all should have data"). Pulled the full "Most Used Perks" table for all remaining 32 killers via NightLight.gg (same browser-navigate method), covering every one of the 44 killers now — no more "not yet ingested" placeholders on any killer detail page. One naming gotcha found and fixed: `slugify("The Onryō")` strips the "ō" as a non-alnum char, producing `the-onry` not `the-onryo` — renamed the data file to match. Survivor perk-breakdown coverage (9/53) is still the remaining gap — same method, not yet run for the rest of the survivor roster.
